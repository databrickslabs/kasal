...
# Frontend test scaffolding not caught by the *.test.* patterns
rm -rf src/frontend/src/__mocks__
rm -f src/frontend/vitest.config.ts src/frontend/src/setupTests.ts src/frontend/src/setupTests.js
echo "  ✓ Removed frontend test scaffolding (__mocks__, setupTests, vitest.config)"

# Shrink marketplace payload (the provider console fetches this tag under a
# 30s backend deadline — see MarketplaceCreateListingResource DEADLINE_EXCEEDED)
echo ""
echo "🗑️  Removing files the deployed app never uses..."

# Only top-level src/docs/*.md are served in-app: the package.json
# prebuild/postbuild scripts copy "docs/*.md" (no subdirectories) into
# frontend_static/docs, and the images those pages reference ship from the
# committed src/frontend/public/docs/images. Everything else in src/docs is
# repo-browsing content.
for d in Blueprints archive examples powerbi images; do
    if [ -d "src/docs/$d" ]; then
        rm -rf "src/docs/$d"
        echo "  ✓ Removed src/docs/$d"
    fi
done

# Dev tooling that plays no part in the Databricks Apps build
# (package.json drives the frontend build; app.yaml runs entrypoint.py)
rm -rf .claude .github docs
rm -f CONTRIBUTING.md CLAUDE.md src/deploy.py src/build.py
echo "  ✓ Removed dev tooling (.claude, .github, docs/, deploy.py, build.py)"

# Large images referenced only by the GitHub README, not by app code
# (in-app icons are favicon/logo192/logo512 per index.html + manifest.json)
rm -f src/frontend/public/kasal-ui-screenshot.png \
      src/frontend/public/Kasal-overview.png \
      src/frontend/public/kasal-mark.png
echo "  ✓ Removed README-only images from src/frontend/public"

# Unreferenced images in the served docs dir: the docs *.md reference only
# images/Kasal-overview.png (keep it); screenshot/logo/favicon.png are dead.
# Empirically this final trim is what got the payload under the marketplace
# backend's 30s CreateListingResource ingestion budget (~19MB passed, ~21MB
# timed out with DEADLINE_EXCEEDED on 2026-07-02).
rm -f src/frontend/public/docs/images/kasal-ui-screenshot.png \
      src/frontend/public/docs/images/logo.png \
      src/frontend/public/favicon.png
echo "  ✓ Removed unreferenced docs/favicon images"

# public/docs/*.md are build artifacts: the package.json prebuild step copies
# src/docs/*.md into frontend/public/docs on every build, so shipping them in
# the tag is pure duplication (~400KB). Keep images/ (not regenerated).
rm -f src/frontend/public/docs/*.md
echo "  ✓ Removed duplicated public/docs markdown (regenerated by prebuild)"

# frontend_static is regenerated on every deploy: the package.json postbuild
# does `rm -rf frontend_static && cp -r frontend/dist frontend_static`, so any
# copy shipped in the tag is deleted and rebuilt on the Apps build container.
# Shipping it is pure dead weight (~430KB).
rm -rf src/frontend_static
echo "  ✓ Removed frontend_static (rebuilt on deploy by postbuild)"

# Dev-only markdown inside the source tree (the in-app docs viewer serves
# only frontend_static/docs, populated from top-level src/docs/*.md)
find src -name "CLAUDE.md" -type f -delete
rm -f src/backend/TODO.md src/frontend/README.md
find src/backend/src src/frontend/src -name "README.md" -type f -delete
rm -f src/backend/src/converters/INBOUND_INTEGRATION_GUIDE.md \
      src/backend/src/converters/MIGRATION.md \
      src/backend/src/converters/README.md \
      src/backend/src/converters/FRONTEND_INTEGRATION_GUIDE.md \
      src/backend/src/converters/COMPLETE_INTEGRATION_SUMMARY.md
find . -name ".gitignore" -type f -delete
echo "  ✓ Removed in-tree dev docs (CLAUDE.md, TODO.md, source READMEs, converter guides, .gitignore)"
...
# Initialize a repo in the (already cleaned) temp directory and commit.
# Cleanup runs before any commit so the secret-scan pre-commit hook only
# sees files that actually ship in the tag.
git init -q
git add -A

...
class TestAgentTraceEventListener:
    ...
    def test_extract_task_info(self, mock_get_queue, setup):
        ...
        # Also need to mock task_id and task_name as None on event itself
        # since _extract_task_info checks direct attributes first
    ...
    def test_update_active_context(self, mock_get_queue, setup):
        ...
        # Update context - method signature: (agent_name, task_name, log_prefix, task_id=None)
        listener._update_active_context("Research Agent", "Analyze data", "[Test]", "task_123")

        # Verify context was updated - context is stored per job_id AND per agent_name
        assert job_id in listener._active_context
        assert "Research Agent" in listener._active_context[job_id]
        assert listener._active_context[job_id]["Research Agent"]['task'] == "Analyze data"
        assert listener._active_context[job_id]["Research Agent"]['task_id'] == "task_123"
...
class TestIntegrationScenarios:
    ...
    def test_agent_execution_with_tool_usage(self, mock_event_bus, mock_get_queue, mock_enqueue_log):
        ...
        # Note: With the current implementation, tool-related outputs are SKIPPED
        # by on_agent_execution_completed to avoid duplicates since dedicated tool
        # event handlers exist. The test may need to verify NO trace was enqueued
        # for tool-related outputs, or we test with non-tool output.

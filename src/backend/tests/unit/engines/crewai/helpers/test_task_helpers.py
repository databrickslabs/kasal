...
class TestGetPydanticClassFromName:
    ...
    async def test_get_pydantic_class_creation_error(self, mock_uow_class):
        ...
        with patch('src.engines.crewai.paths.crew.task_adapter.create_model', side_effect=Exception("Model creation failed")):
    ...
    @pytest.mark.asyncio
    @patch('src.engines.crewai.paths.crew.task_adapter.UnitOfWork')
    @patch('src.engines.crewai.paths.crew.task_adapter.List')
    ...
    async def test_get_pydantic_class_field_assignment_exception(self, mock_uow_class):
        ...
        with patch('src.engines.crewai.paths.crew.task_adapter.List', FailingList()):
            result = await get_pydantic_class_from_name("ExceptionFieldSchema")
...
class TestCreateTask:
    ...
    async def test_create_task_with_output_pydantic(self):
        ...
        with patch('src.core.unit_of_work.UnitOfWork'), \
             patch('src.services.mcp_service.MCPService') as mock_mcp_service, \
             patch('src.engines.crewai.paths.crew.task_adapter.get_pydantic_class_from_name') as mock_get_pydantic, \
             patch('src.engines.crewai.kernel.model_conversion_handler.get_compatible_converter_for_model') as mock_converter:
            mock_mcp_service.from_unit_of_work = AsyncMock(return_value=Mock(
                get_enabled_servers=AsyncMock(return_value=Mock(servers=[]))
            ))
    ...
    async def test_create_task_error_handling(self):
        ...
        with patch('src.core.unit_of_work.UnitOfWork'), \
             patch('src.services.mcp_service.MCPService') as mock_mcp_service, \
             patch('src.engines.crewai.paths.crew.task_adapter.Task', side_effect=Exception("Task creation failed")):
            mock_mcp_service.from_unit_of_work = AsyncMock(return_value=Mock(
                get_enabled_servers=AsyncMock(return_value=Mock(servers=[]))
            ))
    ...
    async def test_create_task_tool_resolution_exception_outer(self):
        ...
        with patch('src.core.unit_of_work.UnitOfWork'), \
            ...
            with patch('src.engines.crewai.kernel.tool_helpers.resolve_tool_ids_to_names') as mock_resolve:
    ...
    async def test_create_task_tool_resolution_outer_exception(self):
        ...
        with patch('src.core.unit_of_work.UnitOfWork'), \
            ...
            with patch('src.engines.crewai.kernel.tool_helpers.resolve_tool_ids_to_names', side_effect=Exception("Tool resolution system failed")):
                task = await create_task(
                    task_key, 
                    task_config, 
                    agent,
    ...
    async def test_create_task_pydantic_output_use_json_approach(self):
        ...
        with patch('src.core.unit_of_work.UnitOfWork'), \
             ...
             patch('src.engines.crewai.paths.crew.task_adapter.get_pydantic_class_from_name') as mock_get_pydantic, \
             patch('src.engines.crewai.kernel.model_conversion_handler.get_compatible_converter_for_model') as mock_converter, \
             patch('src.engines.crewai.kernel.model_conversion_handler.configure_output_json_approach') as mock_configure:
            mock_mcp_service.from_unit_of_work = AsyncMock(return_value=Mock(
                get_enabled_servers=AsyncMock(return_value=Mock(servers=[]))
            ))
            mock_get_pydantic.return_value = TestModel
    ...
    async def test_create_task_uow_initialize_path_coverage(self):
        ...
        with patch('src.core.unit_of_work.UnitOfWork'), \
             patch('src.services.mcp_service.MCPService') as mock_mcp_service, \
             patch('src.engines.crewai.paths.crew.task_adapter.UnitOfWork') as mock_task_uow:
    ...
    async def test_create_task_finally_block_coverage(self):
        ...
        with patch('src.core.unit_of_work.UnitOfWork'), \
             patch('src.services.mcp_service.MCPService') as mock_mcp_service, \
             patch('src.engines.crewai.paths.crew.task_adapter.UnitOfWork') as mock_sync_uow:

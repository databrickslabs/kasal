...
class TestGetPydanticClassFromName:
    ...
    async def test_get_pydantic_class_general_exception(self, mock_uow_class):
        ...
        mock_uow_class.return_value.__aenter__.side_effect = Exception(
...
class TestCreateTask:
    ...
    async def test_create_task_basic(self):
        ...
        agent = Agent(
            role="TestRole", goal="Test Goal", backstory="Test Backstory", verbose=True
        )
    ...
    async def test_create_task_with_tools_resolution(self):
        ...
        mock_tool_service.get_tool_by_id = AsyncMock(
            side_effect=[mock_tool1, mock_tool2]
        )
    ...
    async def test_create_task_with_mcp_sse_server(self):
        ...
        wrapped_tool.name = (
            "TestServer_test_tool"  # The name gets prefixed with server name
        )
        ...
        with (
            ...
            mock_mcp_instance.get_enabled_servers = AsyncMock(
                return_value=mock_servers_response
            )
            mock_mcp_instance.get_server_by_id = AsyncMock(return_value=mock_server)
            mock_mcp_instance.resolve_effective_servers = AsyncMock(
    ...
    async def test_create_task_with_mcp_databricks_server(self):
        ...
        wrapped_tool = Mock(spec=BaseTool)
        wrapped_tool.name = (
            "DatabricksServer_test_tool"  # The name gets prefixed with server name
        )
        wrapped_tool.description = "Test tool description"
        ...
        with (
            ...
            patch("src.services.tools.mcp_adapter.MCPAdapter") as mock_adapter_class,
            patch(
                "src.utils.databricks_auth.get_mcp_auth_headers", new_callable=AsyncMock
            ) as mock_auth,
            patch(
                "src.services.tools.mcp_handler.create_kasal_tool_from_mcp"
            ) as mock_create_tool,
    ...
    async def test_create_task_with_mcp_stdio_server(self):
        ...
        wrapped_tool = Mock(spec=BaseTool)
        wrapped_tool.name = (
            "StdioServer_test_tool"  # The name gets prefixed with server name
        )
        wrapped_tool.description = "Test tool description"
    ...
    async def test_create_task_mcp_service_error(self):
        ...
        with (
            ...
            mock_mcp_service.from_unit_of_work = AsyncMock(
                side_effect=Exception("MCP service failed")
            )
    ...
    async def test_create_task_server_detail_not_found(self):
        ...
        with (
            ...
            mock_mcp_instance = Mock()
            mock_mcp_instance.get_enabled_servers = AsyncMock(
                return_value=Mock(servers=[mock_server])
            )
            mock_mcp_instance.get_server_by_id = AsyncMock(
                return_value=None
    ...
    async def test_create_task_tool_service_no_config_method_coverage(self):
        ...
        mock_tool_factory.create_tool.assert_called_with(
            "Tool1", result_as_answer=False, tool_config_override={}
        )
    ...
    async def test_create_task_tool_resolution_exception(self):
        ...
        mock_tool_service.get_tool_by_id = AsyncMock(
            side_effect=Exception("Tool resolution failed")
        )
    ...
    async def test_create_task_tool_resolution_outer_exception(self):
        ...
        with (
            ...
            with patch(
                "src.services.execution.kernel.tool_helpers.resolve_tool_ids_to_names",
                side_effect=Exception("Tool resolution system failed"),
            ):
                task = await create_task(
                    task_key,
                    task_config,
    ...
    async def test_create_task_with_tools_no_factory(self):
        ...
        with (
            ...
            assert "should be a valid dictionary or instance of BaseTool" in str(
                exc_info.value
            )
    ...
    async def test_create_task_tool_config_with_get_tool_config_by_name(self):
        ...
        mock_tool_service.get_tool_by_id = AsyncMock(return_value=mock_tool)
        mock_tool_service.get_tool_config_by_name = AsyncMock(
            return_value={"result_as_answer": True}
        )
        ...
        with (
            ...
            mock_tool_factory.create_tool.assert_called_once_with(
                "tool1", result_as_answer=True, tool_config_override={}
            )
    ...
    async def test_create_task_guardrail_factory_import_error(self):
        ...
        with (
            ...
            with patch(
                "builtins.__import__",
                side_effect=ImportError("Cannot import guardrail factory"),
            ):
                task = await create_task(task_key, task_config, agent)
    ...
    async def test_create_task_tool_name_filtering(self):
        ...
        mock_tool_service.get_tool_by_id = AsyncMock(
            side_effect=get_tool_by_id_side_effect
        )
    ...
    async def test_create_task_uow_initialize_path_coverage(self):
        ...
        with (
            ...
            assert task.description == "Task description"
            assert (
                task.output_pydantic is not None
            )  # Should have resolved the Pydantic model
    ...
    async def test_create_task_string_tools_coverage(self):
        ...
        with (
            ...
            assert "Input should be a valid dictionary or instance of BaseTool" in str(
                exc_info.value
            )
    ...
    @pytest.mark.skip(
        reason="Test requires refactoring to match implementation changes"
    )
    @pytest.mark.asyncio
    async def test_create_task_server_url_endswith_sse(self):
        ...
        mock_server.server_url = (
            "https://workspace.databricksapps.com/app/sse"  # Already ends with /sse
        )
        mock_server.api_key = "test-key"

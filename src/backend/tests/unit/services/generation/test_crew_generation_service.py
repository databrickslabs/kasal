...
def _crew_complete_patches(
    ...
    class PatchCtx:
        ...
        def __enter__(self):
            p1 = patch("src.services.generation.crew.complete.ToolService")
            p2 = patch.object(service, "_get_tool_details", new_callable=AsyncMock)
            p3 = patch.object(
                service, "_prepare_prompt_template", new_callable=AsyncMock
            )
            p4 = patch("src.services.generation.crew.complete.LLMManager")
            p5 = patch("src.services.generation.crew.complete.robust_json_parser")
...
class TestPreparePromptTemplate:
    ...
    async def test_template_not_found_raises(self):
        with patch("src.services.generation.crews.TemplateService") as ts:
...
class TestCreateCrewComplete:
    ...
    async def test_happy_path(self):
        ...
        with (
            patch("src.services.generation.crew.complete.ToolService"),
            patch.object(
            ...
            patch("src.services.generation.crew.complete.LLMManager") as lm,
    ...
    async def test_outer_exception_reraises(self):
        ...
        with patch(
            "src.services.generation.crew.complete.ToolService",
...
class TestProgressiveGeneration:
    ...
    async def test_generate_crew_plan_template_not_found(self):
        ...
        with patch("src.services.generation.crew.progressive.TemplateService") as ts:
            ts.get_effective_template_content = AsyncMock(return_value=None)
    ...
    def _progressive_patches(
        ...
        class ProgressivePatchCtx:
            ...
            def __enter__(self):
                ...
                p_sse = patch("src.services.generation.crew.progressive.sse_manager")
                ...
                p_agent_svc = patch(
                    "src.services.generation.crew.progressive.AgentGenerationService"
                )
                # Patch TaskGenerationService
                p_task_svc = patch(
                    "src.services.generation.crew.progressive.TaskGenerationService"
                )

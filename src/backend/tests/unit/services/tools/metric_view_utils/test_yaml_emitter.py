...
class TestEmitYaml:
            ...
            joins=[
                {
                    "name": "dim_geo",
                    "source": "cat.sch.dim",
                    "join_on": "source.key = dim_geo.key",
                }
            ],
            dimensions=[],
            measures=[
    ...
    def test_dangerous_sql_blocked(self):
        ...
        spec = MetricViewSpec(
            ...
            measures=[
                TranslationResult(
                    measure_name="safe_val",
                    original_name="Safe Val",
                    sql_expr="SUM(source.val)",
                    is_translatable=True,
                    skip_reason="",
                    dax_expression="",
                ...
                TranslationResult(
                    measure_name="evil",
                    original_name="Evil",
                    sql_expr="SUM(source.val); DROP TABLE users",
                    is_translatable=True,
                    skip_reason="",
                    dax_expression="",
                    confidence="high",
    ...
    def test_dax_measures_emitted(self):
        ...
        spec = MetricViewSpec(
            ...
            measures=[
                TranslationResult(
                    measure_name="base_val",
                    original_name="Base Val",
                    sql_expr="SUM(source.val)",
                    is_translatable=True,
                    skip_reason="",
                    dax_expression="",
                ...
                TranslationResult(
                    measure_name="dax_ratio",
                    original_name="DAX Ratio",
    ...
    def test_switch_measures_emitted(self):
        ...
        spec = MetricViewSpec(
            ...
            measures=[
                ...
                TranslationResult(
                    measure_name="sw_branch",
                    original_name="SW Branch",
                    sql_expr="SUM(source.val) FILTER (WHERE source.type = 'A')",
                    is_translatable=True,
                    skip_reason="SWITCH branch: type=A",
                    dax_expression="",
                    confidence="high",
    ...
    def test_untranslatable_listed_as_comments(self):
        ...
        spec = MetricViewSpec(
            ...
            untranslatable=[
                TranslationResult(
                    measure_name="complex",
                    original_name="Complex Measure",
                    sql_expr=None,
...
class TestMeasureUsageSurfacing:
    ...
    def test_untranslatable_block_sorted_desc_and_annotated(self):
        def _todo(name, ref):
            return TranslationResult(
                measure_name=name.lower(),
                original_name=name,
                sql_expr=None,
                is_translatable=False,
                skip_reason="TODO gap",
...
class TestCheckDangerousSql:
    ...
    def test_safe_measure_ref(self):
        assert (
            _check_dangerous_sql("MEASURE(total_sales) - MEASURE(total_cost)") is True
        )

    def test_safe_filter(self):
        assert (
            _check_dangerous_sql("SUM(source.val) FILTER (WHERE source.type = 'A')")
            is True
        )
    ...
    def test_safe_case_when(self):
        assert (
            _check_dangerous_sql("CASE WHEN source.x > 0 THEN SUM(source.y) ELSE 0 END")
            is True
        )
    ...
    def test_dangerous_union_info_schema(self):
        assert (
            _check_dangerous_sql("UNION SELECT table_name FROM information_schema")
            is False
        )
...
class TestDimensionAndJoinDedup:
    ...
    def test_duplicate_join_names_collapsed(self):
        joins = [
            {
                "name": "dim_plant",
                "source": "cat.sch.plant_a",
                "on": "source.p = dim_plant.p",
            },
            {
                "name": "dim_plant",
                "source": "cat.sch.plant_b",
                "on": "source.p = dim_plant.p",
            },
        ]
    ...
    def test_colliding_dimension_dropped(self):
        spec = MetricViewSpec(
            ...
            dimensions=[
                ...
                },  # collides with measure
                {"name": "region", "expr": "dim_geo.region"},  # keep
            ],
            measures=[
                TranslationResult(
                    measure_name="kbi_value",
                    original_name="KBI Value",
                    sql_expr="SUM(source.kbi_value)",
                    is_translatable=True,
                    skip_reason="kbi",
                    dax_expression="SUM(T[kbi_value])",
                    confidence="high",
                    category="base",
                )
            ],
...
class TestUntranslatableCategorization:
    ...
    def test_prior_year_grouped_with_reason(self):
        y = self._emit(
            [self._u("NSR PY", "CALCULATE(SUM(t[nsr]), SAMEPERIODLASTYEAR(cal[d]))")]
        )
    ...
    def test_dynamic_selector_grouped(self):
        y = self._emit(
            [
                self._u(
                    "Mega", "var x=SELECTEDVALUE(m[k]) return SWITCH(TRUE(), x=1, [A])"
                )
            ]
        )
    ...
    def test_treatas_dispatch_grouped(self):
        y = self._emit(
            [self._u("KPI Dispatch", "CALCULATE([M], TREATAS({kbiName}, T[Name]))")]
        )
    ...
    def test_allexcept_fixed_lod_grouped(self):
        y = self._emit(
            [self._u("YearWeight", "CALCULATE(SUM(t[weight]), ALLEXCEPT(t, t[Year]))")]
        )

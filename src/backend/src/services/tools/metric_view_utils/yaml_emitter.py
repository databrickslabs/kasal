...
def _check_dangerous_sql(expr: str) -> bool:
    ...
    _DANGEROUS_PATTERNS = re.compile(
        ...
        r"|\$\$" r"|\bUNION\s+SELECT\s+.*\bFROM\s+information_schema\b" r")",
        re.IGNORECASE,
    )
...
def _check_metadata_limits(spec: MetricViewSpec) -> list[str]:
    ...
    if len(spec.dimensions) > MAX_DIMENSIONS:
        warnings.append(
            f"Too many dimensions: {len(spec.dimensions)} > {MAX_DIMENSIONS}"
        )
    ...
    for m in spec.measures:
        if m.sql_expr and len(m.sql_expr) > MAX_EXPR_LENGTH:
            warnings.append(
                f"Expression too long for {m.measure_name}: {len(m.sql_expr)} > {MAX_EXPR_LENGTH}"
            )
    return warnings
...
def _categorize_untranslatable(measure) -> tuple[str, str]:
    ...
    if (
        ...
        or "color" in name
        or "format" in reason.lower()
        or "display" in reason.lower()
    ):
        return ("display artifact", "formatting/label only — not a data measure")
    if (
        re.search(r"SAMEPERIODLASTYEAR|DATEADD|PARALLELPERIOD|PREVIOUSYEAR", du)
    ...
    if "SELECTEDVALUE" in du or re.search(
        r"\bF_(START|END)_DATE\b|CUR_MONTH|CUR_YR", du
    ):
        return (
...
def emit_yaml(
    ...
    if spec.joins:
        ...
        for j in spec.joins:
            tbl_short = (
                j["source"].split(".")[-1] if "." in j["source"] else j["source"]
            )
    ...
    for m in base_measures + dax_measures + switch_measures + implicit_measures:
        if m.sql_expr:
            for alias_name, phys_name in _COLUMN_ALIAS_MAP.items():
                m.sql_expr = re.sub(
                    rf"\bsource\.{re.escape(alias_name)}\b",
                    f"source.{phys_name}",
                    m.sql_expr,
                )
    ...
    for measure_list in (base_measures, dax_measures, switch_measures, implicit_measures):
        ...
        for i, m in enumerate(measure_list):
            if m.sql_expr and not _check_dangerous_sql(m.sql_expr):
                logger.warning(
                    f"[SECURITY] Dangerous SQL detected in {m.measure_name}: {m.sql_expr[:100]}"
                )
    ...
    if spec.source_filter and not _check_dangerous_sql(spec.source_filter):
        logger.warning(
            f"[SECURITY] Dangerous SQL in source_filter: {spec.source_filter[:100]}"
        )
        spec.source_filter = ""
    ...
    def _has_invalid_alias(expr: str) -> str | None:
        ...
        for alias_m in re.finditer(r"\b(\w+)\.(\w+)", expr):
            ...
            if (
                alias not in _declared_aliases
                and alias.lower() not in _declared_aliases
            ):
                return alias
        return None
            ...
            for i, m in enumerate(measure_list):
                if m.sql_expr:
                    ...
                    if invalid_refs:
                        m.is_translatable = False
                        m.skip_reason = (
                            f"References non-existent measures: {invalid_refs}"
                        )
                        m.sql_expr = None
                        spec.untranslatable.append(m)
                        drop_idx.append(i)
    ...
    if _known_source_cols:
        ...
        for measure_list in (dax_measures, switch_measures, implicit_measures):
            ...
            for i, m in enumerate(measure_list):
                if m.sql_expr and "FILTER" in m.sql_expr:
                    for fc_m in re.finditer(
                        r"FILTER\s*\(WHERE\s+([^)]+)\)", m.sql_expr
                    ):
                        ...
                        bare_in_filter = {
                            c for c in bare_in_filter if c.upper() not in _SQL_KW_FV
                        }
                        unknown_filter_cols = bare_in_filter - _all_known_filter_cols
                        if unknown_filter_cols:
                            m.is_translatable = False
                            m.skip_reason = (
                                f"FILTER references unknown columns: "
    ...
    if spec.source_sql:
        ...
        first_arm = re.split(
            r"\bUNION\b", spec.source_sql, maxsplit=1, flags=re.IGNORECASE
        )[0]
        sel_m = re.match(
    ...
    spec.dimensions = [
        d
        for d in spec.dimensions
        if d.get("name") and d.get("expr") and d["expr"] != "source."
    ]
    ...
    if spec.dimensions:
        ...
        if _measure_names:
            spec.dimensions = [
                d for d in spec.dimensions if d["name"] not in _measure_names
            ]
    ...
    if base_measures:
        lines.append(
            f"# \u2500\u2500\u2500 Base Measures ({len(base_measures)}) "
            + "\u2500" * (58 - len(str(len(base_measures))))
        )
        lines.append("")
    ...
    if dax_measures:
        lines.append(
            f"  # \u2500\u2500\u2500 DAX-Translated Measures ({len(dax_measures)}) "
            + "\u2500" * (50 - len(str(len(dax_measures))))
        )
        lines.append("")
            ...
            if not dax_display:
                ...
                if dax_display:
                    if m.measure_name.endswith(budget_suffix):
                        ...
                        for m2 in dax_measures
                        if m2.measure_name.endswith(budget_suffix)
                    ):
                        dax_display += " (Actual)"
            if dax_display:
    ...
    if switch_measures:
        lines.append(
            f"  # \u2500\u2500\u2500 SWITCH-Decomposed Measures ({len(switch_measures)}) "
            + "\u2500" * (46 - len(str(len(switch_measures))))
        )
        lines.append("")
        for m in switch_measures:
            lines.append(f"  - name: {m.measure_name}")
    ...
    if spec.untranslatable:
        lines.append(
            f"  # \u2500\u2500\u2500 Not emitted as measures ({len(spec.untranslatable)}) \u2014 grouped by reason \u2500\u2500\u2500"
        )

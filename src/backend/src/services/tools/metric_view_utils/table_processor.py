...
from .utils import col_to_readable, run_async, spark_sql_compat, to_snake_case
...
def _detect_common_exclusions(dax_measures: list[dict], source_cols: set) -> list[str]:
    ...
    n_with_dax = 0
    _ne = re.compile(r"""(?:'[^']+'|\w+)\[(\w+)\]\s*<>\s*("?[\w./-]+"?|'[^']*')""")
    _notin = re.compile(
        r"""NOT\s+(?:'[^']+'|\w+)\[(\w+)\]\s+IN\s*[\{(]([^})]+)[\})]""", re.IGNORECASE
    )
...
_ARTIFACT_SKIP_KEYWORDS = (
    ...
    "ISFILTERED",
    "DIVIDE over PBI artifacts",
    "PY/DIVIDE over PBI artifacts",
    "BLANK() placeholder",
    "cross-table",
...
def process_table(
    ...
    *,
    ...
    joins = ctx.join_detector.detect(
        table_key, dax_measures, table_info, inner_dim_joins=ctx.inner_dim_joins
    )

    # 3b. Auto-detect fact-to-fact joins for cross-table DIVIDEs
    fact_joins = ctx.join_detector.detect_fact_joins(
        table_key, dax_measures, table_info
    )
    ...
    for m in dax_measures:
        ...
        for match in userel_pattern.finditer(dax):
            ...
            for irel in inactive_rels:
                if (
                    irel["from_column"].lower() == fact_col.lower()
    ...
    for m in translated:
        if (
            m.window_spec
            and m.window_spec.get("order") == "date_key"
            and period_dim != "date_key"
        ):
            m.window_spec["order"] = period_dim

    # 5a-fix2. Drop window measures on INT period columns (configurable).
    _INT_PERIOD_DIMS = set(
        ctx.config.get("int_period_dims", ["fiscper", "fiscal_year_period"])
    )
    ...
    for cycle_group in dep_graph.get("cycles", []):
        ...
        for m in untranslatable:
            if m.original_name in cycle_names:
                m.skip_reason = (
                    f"Circular dependency: {' ↔ '.join(sorted(cycle_group)[:3])}"
                )
    ...
    for _pass2_iter in range(5):
        ...
        sorted_untranslatable = sorted(
            untranslatable, key=lambda m: _topo_priority.get(m.original_name, 999)
        )
            ...
            if measure_refs and all(
                ...
                for line in expr.split("\n"):
                    ...
                    if re.match(r"^var\s+(std|etd|x|y)\b", s, re.IGNORECASE):
                        continue
                    if re.match(
                        r"^var\s+\w+\s*=\s*CALCULATE\s*\(\s*\[(F_Start|F_End|PY_Start|PY_End)",
                        s,
                        re.IGNORECASE,
                    ):
                        continue
                ...
                while "DIVIDE(" in expr.upper():
                    ...
                    expr = (
                        expr[:d_start]
                        + f"{num.strip()} / NULLIF({den.strip()}, 0)"
                        + expr[d_end:]
                    )
                ...
                expr = re.sub(
                    r",\s*SAMEPERIODLASTYEAR\([^)]+\)", "", expr, flags=re.IGNORECASE
                )
                # Strip leftover CALCULATE() wrappers (bare, no filter)
                expr = re.sub(
                    r"\bCALCULATE\(\s*(MEASURE\([^)]+\))\s*\)",
                    r"\1",
                    expr,
                    flags=re.IGNORECASE,
                )
        ...
        try:
            ...
            if llm_translated:
                ...
                untranslatable = [
                    m
                    for m in untranslatable
                    if m.measure_name not in {t.measure_name for t in llm_translated}
                ]
                logger.info(
                    f"[DAX_LLM] {len(llm_translated)} measures translated via LLM fallback for {table_key}"
                )
    ...
    for _cascade_pass in range(5):
        artifact_names = {
            m.original_name
            for m in untranslatable
            if any(kw in m.skip_reason for kw in _ARTIFACT_SKIP_KEYWORDS)
        }
        changed = 0
        for m in untranslatable:
            cascade_skip = (
            ...
            if cascade_skip:
                ...
                if (
                    refs
                    and all(r in artifact_names or r in base_names for r in refs)
                    and any(r in artifact_names for r in refs)
                ):
    ...
    if table_key in switch_decomps:
        ...
        if isinstance(entries, list):
            ...
            for defn in entries:
                ...
                if (
                    not _is_real_switch_decomp(defn)
                    and defn["name"] in _real_translated
                ):
                    continue
                if (
                    defn["name"] not in base_names
                    or defn["name"] in dax_translated_names
                ):
                    switch_measures.append(
                        build_switch_measure_fn(
                            defn, filter_sets=ctx.translator.filter_sets
                        )
                    )
        ...
        if isinstance(entries, list):
            ...
            for m in translated:
                ...
                if identity_match:
                    continue
                if (
                    m.measure_name in switch_names
                    and m.measure_name in added_switch_names
                ):
                    continue
                    ...
                    if jk.get("alias") == alias:
                        ...
                        if dim_info and dim_info.source_table:
                            ...
                            for fact_alias, fact_src in fact_dims.items():
                                if (
                                    fact_alias.lower() == alias
                                    or dim_name.lower() in fact_alias.lower()
                                ):
                        ...
                        join_on = (
                            alt_on
                            if alt_on
                            else f'source.{join_key} = {alias}.{jk.get("dim_key", join_key)}'
                        )
                        joins.append(
                            {
                                "name": alias,
                        ...
                        logger.info(
                            f"[{table_key}] Auto-added join '{alias}' → {source_table}"
                        )
        ...
        def _rewrite(
            ...
            for hit in _fs.finditer(expr):
                ...
                if hit.group(1):  # = 'CODE'
                    code = hit.group(1)
                    replacement = (
                        f'SUM({_alias}.{_pkm.get(code, f"sc_{code.lower()}")})'
                    )
                else:  # IN ('X','Y')
                    codes = re.findall(r"'([A-Z0-9]+)'", hit.group(2))
                    parts = [
                        f'SUM({_alias}.{_pkm.get(c, f"sc_{c.lower()}")})' for c in codes
                    ]
                    inner = " + ".join(parts) if parts else hit.group(0)
                    replacement = f"({inner})" if len(parts) > 1 else inner
                result_parts.append(replacement)
                filter_paren = expr.index(
                    "(", hit.start() + expr[hit.start() :].upper().find("FILTER") + 6
                )
    ...
    if fc_warnings:
        ctx.filter_warnings.extend([f"{table_key}: {w}" for w in fc_warnings])
    ...
    if table_info.static_filters:
        ...
        for col_m in re.finditer(
            r"\b(\w+)\s+AS\s+`?(\w+)`?", table_info.full_sql, re.IGNORECASE
        ):
        ...
        for filt in table_info.static_filters:
            ...
            real_bare = {
                c
                for c in bare_cols
                if c.upper() not in _SQL_KEYWORDS and not c.isdigit()
            }
    ...
    if (
        ...
        if as_match:
            ...
            first_kw = (
                candidate.lstrip().split()[0].upper() if candidate.strip() else ""
            )
    ...
    if ctx.scan_data and table_key in ctx.scan_data:
        ...
        post_processor = SqlPostProcessor(
            ...
            expand_re_version=bool(
                ctx.config.get("parameter_defaults", {}).get("RE_Version_ranges")
            ),
        )
        ...
        first_arm = re.split(r"\bUNION\b", final_sql, maxsplit=1, flags=re.IGNORECASE)[
            0
        ]
        sel_m = re.match(
            r"\s*SELECT\s+(.*?)\s+FROM\s+", first_arm, re.IGNORECASE | re.DOTALL
        )
        if sel_m:
            for col_part in MTransformFolder._split_select_columns(sel_m.group(1)):
                col_part = col_part.strip()
                if not col_part or re.match(
                    r"^\s*(SUM|AVG|COUNT|MIN|MAX)\s*\(", col_part, re.IGNORECASE
                ):
                    continue
                ...
                if (
                    col_name.lower() not in existing_dim_names
    ...
    comment_lines = [
        ...
        + (
            f" ({cross_table_translated_count} cross-table)"
            if cross_table_translated_count
            else ""
        ),
    ...
    if untranslatable:
        ...
        comment_lines.append(
            "Untranslatable (sorted by usage — fix high-impact gaps first):"
        )
        for r in sorted(untranslatable, key=lambda m: m.referenced_by, reverse=True):
            _usage = (
                f" [referenced by {r.referenced_by}"

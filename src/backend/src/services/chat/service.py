...
# The terminal steps of a run log here too: this module's logger does not reach
# system.log, and a run that answered but never reached COMPLETED was invisible.
_system_log = LoggerManager.get_instance().system
...
class LightAgentService:
    ...
    async def run_light_agent_execution(
        ...
        try:
            ...
            async def _flush_and_close_traces(timeout: float) -> None:
                ...
                # Bounded for the same reason as the flush: the status write
                # comes next, and nothing in between may wait forever.
                await _trace_writer.close(timeout=5)
    ...
    async def _conversation_preamble(
        ...
        how far each is cut — lives in ``conversation_preamble``; this stays a
        method so the call site and its tests keep one door."""
        from src.services.chat.conversation_preamble import (
            build_conversation_preamble,
        )

        return await build_conversation_preamble(config, group_context, group_id, log)

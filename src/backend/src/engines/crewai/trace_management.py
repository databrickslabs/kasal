                    ...
                    if batch:
                        ...
                        for idx, trace_data in enumerate(batch):
                            try:
                                ...
                                if job_exists:
                                    # Broadcast task lifecycle events via SSE for real-time UI updates
                                    ...
                                    if event_type in ("task_started", "task_completed", "task_failed"):
                                        ...
                                        try:
                                            event = SSEEvent(
                                                data=sse_trace_data,
                                                event="trace",
                                                id=f"{job_id}_{event_type}_{datetime.now().timestamp()}"
                                            )
                                            sent_count = await sse_manager.broadcast_to_job(job_id, event)
                                            logger.info(f"[TraceManager._trace_writer_loop] Broadcasted {event_type} SSE to {sent_count} clients for job {job_id}")
                                        except Exception as sse_error:
                                            logger.warning(f"[TraceManager._trace_writer_loop] Failed to broadcast {event_type} SSE: {sse_error}")

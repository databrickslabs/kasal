...
By the end of this tutorial you'll have built and run a multi-agent blog-production workflow in Kasal, then customized it from the Catalog, with no admin setup required.
...
- Fill:
  ...
  - Description: Research 5 trending topics in {industry} and provide 2 to 3 sentence summaries per topic.
...
- Add Task: Draft Article
  - Description: Write a blog post (800 to 1200 words) from the topic research.
...
Callouts
- Tip: Small prompt edits can produce large quality differences, so change one thing at a time.

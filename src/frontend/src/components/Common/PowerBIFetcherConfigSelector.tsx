/**
 * Power BI Semantic Model Fetcher Configuration Selector Component
 *
 * Provides configuration UI for the Power BI Semantic Model Fetcher Tool.
 * Fetches table/column metadata from Power BI semantic models via DAX INFO functions.
 */
...
export interface PowerBIFetcherConfig {
  mode?: 'static' | 'dynamic';
  workspace_id?: string;
  dataset_id?: string;
  report_id?: string;
...
interface PowerBIFetcherConfigSelectorProps {
  ...
  onChange: (config: PowerBIFetcherConfig) => void;
  disabled?: boolean;
}

export const PowerBIFetcherConfigSelector: React.FC<PowerBIFetcherConfigSelectorProps> = ({
  value = {},
  onChange,
  disabled = false
}) => {
  ...
  const handleFieldChange = (field: keyof PowerBIFetcherConfig, fieldValue: string | boolean | number) => {
    onChange({
      ...value,
      [field]: fieldValue
    });
  ...
  const handleAuthMethodChange = (_event: React.MouseEvent<HTMLElement>, newMethod: PowerBIAuthMethod | null) => {
    if (newMethod !== null) {
      const updatedConfig: PowerBIFetcherConfig = {
        ...value,
  ...
  const handleModeChange = (_event: React.MouseEvent<HTMLElement>, newMode: 'static' | 'dynamic' | null) => {
    if (newMode !== null) {
      ...
      if (newMode === 'dynamic') {
        ...
        updatedConfig.enable_info_columns = value.enable_info_columns || false;
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        <Alert severity="info" sx={{ mb: 1 }}>
          ...
          <Box sx={{ mt: 1.5, p: 1, bgcolor: 'rgba(0,0,0,0.04)', borderRadius: 1 }}>
            ...
            <Typography variant="caption" color="text.secondary" sx={{ mt: 0.5, display: 'block' }}>
              Optional: <Chip label="report_id" size="small" variant="outlined" sx={{ ml: 0.5 }} />
            </Typography>
          </Box>
        ...
        <>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <TextField
              ...
              required
              fullWidth
              helperText="Semantic model/dataset ID to fetch metadata from"
              size="small"
            />
            <TextField
              ...
              disabled={disabled}
              fullWidth
              helperText="Optional report ID for extracting default filters"
              size="small"
            />
          </Box>
        ...
        <AccordionDetails>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            ...
            <FormControlLabel
              control={
                <Checkbox
                  checked={value.enable_info_columns || false}
                  onChange={(e) => handleFieldChange('enable_info_columns', e.target.checked)}
                  disabled={disabled}
                  size="small"
                />
              }
              label={<Typography variant="body2">Enable INFO columns (additional metadata)</Typography>}
            />
      ...
      <Alert severity="info" variant="outlined" sx={{ mt: 1 }}>
        ...
        <Typography variant="caption">
          Fetches table and column metadata from Power BI semantic models using DAX INFO functions
          (<code>INFO.VIEW.TABLES()</code>, <code>INFO.VIEW.COLUMNS()</code>). Provides structured
          schema information for downstream analysis.
        </Typography>
        <Box component="ul" sx={{ pl: 2, mt: 0.5, mb: 0, fontSize: '0.75rem' }}>
          <li>Extracts all tables, columns, data types, and expressions</li>
          <li>Optionally includes INFO column metadata</li>
          <li>Filters system tables (LocalDateTable, DateTableTemplate)</li>
          <li>Supports JSON and Markdown output formats</li>
        </Box>
      </Alert>
    </Box>
  );

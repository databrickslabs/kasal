...
Workspace admins can browse and register Databricks-hosted MCP servers from the chat picker. The catalog is served by `GET /mcp/databricks/available` and is admin-only. Each managed server is registered as a Kasal MCP server with `auth_type=databricks_spn` and `server_type=streamable`.
...
- Unity Catalog Functions: run the functions in a catalog and schema. The picker offers the catalog and schema from the workspace's Databricks configuration when configured (`/api/2.0/mcp/functions/{catalog}/{schema}`), plus the built-in `system.ai` functions such as `python_exec` (`/api/2.0/mcp/functions/system/ai`).
...
- When a workspace (group) context is present, a server created via `POST /mcp/servers` is scoped to that workspace. Without a workspace context it is created as a base (global) server.
...
MCP follows a two-tier model, the same pattern Kasal uses for Models: a global (system-admin) registry plus per-workspace overrides. The key distinction is between two boolean columns on `mcp_servers`:

- `enabled`: controls availability. For a base server (where `group_id` is `NULL`), `enabled` means "available to all workspaces". For a workspace-scoped row, `enabled` is that workspace's own on/off state.
- `global_enabled`: a separate flag meaning "auto-attach this server to every crew" as a baseline, independent of availability.

Do not confuse the two: a server being available to workspaces is governed by `enabled`, not by `global_enabled`.
...
- Workspace admins: workspace-scoped overrides, plus creating servers scoped to their workspace.
